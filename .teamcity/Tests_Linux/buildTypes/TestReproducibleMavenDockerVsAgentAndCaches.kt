...
object TestReproducibleMavenDockerVsAgentAndCaches : BuildType({
    ...
    steps {
        script {
            name = "Check artifacts Kotlin_BuildPlayground_Titan_MavenArtifactsDocker <-> Kotlin_BuildPlayground_Titan_CompilerDistAndMavenArtifacts"
            executionMode = BuildStep.ExecutionMode.RUN_ON_FAILURE
            scriptContent = """
                #!/bin/bash
                set -x
                export JAVA_HOME=%env.JDK_11_0%
                ./trusted-binaries-equivalence-checker/bin/trusted-binaries-equivalence-checker --exceptions="%equality-exceptions%"  "Kotlin_BuildPlayground_Titan_MavenArtifactsDocker/${MavenArtifactsDocker.depParamRefs["reproducible.maven.artifact"]}" "Kotlin_BuildPlayground_Titan_CompilerDistAndMavenArtifacts/${CompilerDistAndMavenArtifacts.depParamRefs["reproducible.maven.artifact"]}"
            """.trimIndent()
    ...
    dependencies {
        dependency(_Self.buildTypes.CompilerDistAndMavenArtifacts) {
            ...
            artifacts {
                artifactRules = "+:${CompilerDistAndMavenArtifacts.depParamRefs["reproducible.maven.artifact"]}=>Kotlin_BuildPlayground_Titan_CompilerDistAndMavenArtifacts"
            }
        }
        dependency(_Self.buildTypes.MavenArtifactsDocker) {
            ...
            artifacts {
                artifactRules = "+:${MavenArtifactsDocker.depParamRefs["reproducible.maven.artifact"]}=>Kotlin_BuildPlayground_Titan_MavenArtifactsDocker"
            }
        }
