...
object TestReproducibleCompilerDockerVsAgent : BuildType({
    ...
    steps {
        script {
            name = "Check artifacts Kotlin_BuildPlayground_Titan_CompilerArtifactsInDocker <-> Kotlin_BuildPlayground_Titan_CompilerArtifacts"
            executionMode = BuildStep.ExecutionMode.RUN_ON_FAILURE
            scriptContent = """
                #!/bin/bash
                set -x
                export JAVA_HOME=%env.JDK_11_0%
                ./trusted-binaries-equivalence-checker/bin/trusted-binaries-equivalence-checker   "Kotlin_BuildPlayground_Titan_CompilerArtifactsInDocker/${CompilerArtifactsInDocker.depParamRefs["kotlin.compiler.zip.name"]}" "Kotlin_BuildPlayground_Titan_CompilerArtifacts/${CompilerArtifacts.depParamRefs["kotlin.compiler.zip.name"]}"
            """.trimIndent()
    ...
    dependencies {
        dependency(_Self.buildTypes.CompilerArtifacts) {
            ...
            artifacts {
                artifactRules = "+:${CompilerArtifacts.depParamRefs["kotlin.compiler.zip.name"]}=>Kotlin_BuildPlayground_Titan_CompilerArtifacts"
            }
        }
        dependency(_Self.buildTypes.CompilerArtifactsInDocker) {
            ...
            artifacts {
                artifactRules = "+:${CompilerArtifactsInDocker.depParamRefs["kotlin.compiler.zip.name"]}=>Kotlin_BuildPlayground_Titan_CompilerArtifactsInDocker"
            }
        }
