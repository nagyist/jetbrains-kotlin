...
object TestReproducibleDeployMavenVsDocker : BuildType({
    ...
    steps {
        script {
            name = "Check artifacts Kotlin_BuildPlayground_Titan_DeployKotlinMavenArtifacts <-> Kotlin_BuildPlayground_Titan_MavenArtifactsDocker"
            executionMode = BuildStep.ExecutionMode.RUN_ON_FAILURE
            scriptContent = """
                #!/bin/bash
                set -x
                export JAVA_HOME=%env.JDK_11_0%
                ./trusted-binaries-equivalence-checker/bin/trusted-binaries-equivalence-checker   "Kotlin_BuildPlayground_Titan_DeployKotlinMavenArtifacts/${DeployKotlinMavenArtifacts.depParamRefs["reproducible.maven.artifact"]}" "Kotlin_BuildPlayground_Titan_MavenArtifactsDocker/${MavenArtifactsDocker.depParamRefs["reproducible.maven.artifact"]}"
            """.trimIndent()
    ...
    dependencies {
        dependency(Deploy.buildTypes.DeployKotlinMavenArtifacts) {
            ...
            artifacts {
                artifactRules = "+:${DeployKotlinMavenArtifacts.depParamRefs["reproducible.maven.artifact"]}=>Kotlin_BuildPlayground_Titan_DeployKotlinMavenArtifacts"
            }
        }
