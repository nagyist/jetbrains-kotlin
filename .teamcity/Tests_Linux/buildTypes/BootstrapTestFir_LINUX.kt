...
object BootstrapTestFir_LINUX : BuildType({
    ...
    params {
        param("gradleParameters", "%globalGradleParameters% -Dkotlin.daemon.jvm.options=-Xmx4096m -Pkotlin.build.useFir=true -PdeployVersion=%DeployVersion% -Pbuild.number=%build.number% -Pbootstrap.local=true -Pbootstrap.local.version=${CompilerDistAndMavenArtifacts.depParamRefs["DeployVersion"]} -Pbootstrap.local.path=%teamcity.build.checkoutDir%/bootstrap")
