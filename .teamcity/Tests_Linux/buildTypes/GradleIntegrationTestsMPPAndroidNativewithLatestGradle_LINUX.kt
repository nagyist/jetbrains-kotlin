...
object GradleIntegrationTestsMPPAndroidNativewithLatestGradle_LINUX : BuildType({
    name = "🐧 Gradle Integration Tests MPP Android Native with Latest Gradle (Linux)"
    ...
    steps {
        ...
        gradle {
            ...
            tasks = ":kotlin-gradle-plugin-integration-tests:testAdvanceGradleVersion"
            buildFile = "build.gradle.kts"
            workingDir = "%teamcity.build.checkoutDir%/kotlin"
            gradleParams = "%gradleParameters% --parallel -Pgradle.integration.tests.split.tasks=false -Pkotlin.build.disable.verification.tasks=true"
            enableStacktrace = false
            jdkHome = "%env.JDK_11_0%"
        }
        gradle {
