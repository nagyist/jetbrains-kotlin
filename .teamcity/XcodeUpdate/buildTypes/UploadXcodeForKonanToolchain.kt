...
object UploadXcodeForKonanToolchain : BuildType({
            ...
            scriptContent = """
                ...
                def main():
                    ...
                    with TemporaryDirectory() as temporary_directory:
                        ...
                        subprocess.run(args=['/usr/bin/curl', '-vfsSL', xcode_xip_url, '-o', xip_path])
                        print('Unzipping...')
                        subprocess.run(args=['/usr/bin/xip', '--expand', xip_path], cwd=temporary_directory_path)
                        ...
                        for file in files_to_upload:
                            ...
                            subprocess.run(args=['/usr/bin/curl', '-vfsSL', '-X', 'PUT', '-T', file,
                ...
                def accept_license(xcode_path: Path):
                    subprocess.run(
                        args=['sudo', '/usr/bin/xcodebuild', '-license', 'accept'],
                        env={'DEVELOPER_DIR': xcode_path}
                    )
    ...
    requirements {
        ...
        startsWith("teamcity.agent.name", "titan-upload-xcode")
    }
})
