...
object DeployKotlinMavenArtifacts : BuildType({
    ...
    params {
        ...
        param("reproducible.maven.artifact", "reproducible-maven-Kotlin_BuildPlayground_Titan_DeployKotlinMavenArtifacts-%DeployVersion%.zip")
    ...
    steps {
        ...
        gradle {
            ...
            workingDir = "%teamcity.build.checkoutDir%/kotlin"
            gradleParams = "%gradleParameters% --no-parallel -PdeployVersion=%DeployVersion% -Pversions.kotlin-native=%versions.kotlin-native% -Dmaven.repo.local=%teamcity.maven.local.repository.path% -Dorg.gradle.internal.publish.checksums.insecure=true -PsigningRequired=true -Psigning.gnupg.executable=gpg -Psigning.gnupg.keyName=%sign.key.id.new% -Psigning.gnupg.homeDir=%teamcity.build.checkoutDir%/kotlin/libraries/.gnupg -Psigning.gnupg.passphrase=%sign.key.passphrase.new% --no-scan --no-build-cache -Pdeploy-repo=%deploy-repo% -Pdeploy-url=%deploy-url%"
        ...
        gradle {
            ...
            buildFile = "build.gradle.kts"
            workingDir = "%teamcity.build.checkoutDir%/kotlin"
            gradleParams = "%gradleParameters% --no-parallel -PdeployVersion=%DeployVersion% -Pversions.kotlin-native=%versions.kotlin-native% -Dmaven.repo.local=%teamcity.maven.local.repository.path% -Dorg.gradle.internal.publish.checksums.insecure=true -PsigningRequired=true -Psigning.gnupg.executable=gpg -Psigning.gnupg.keyName=%sign.key.id.new% -Psigning.gnupg.homeDir=%teamcity.build.checkoutDir%/kotlin/libraries/.gnupg -Psigning.gnupg.passphrase=%sign.key.passphrase.new% --no-scan --no-build-cache -Pdeploy-repo=local -Pdeploy-url=file://%teamcity.build.checkoutDir%/kotlin/build/repo"
            enableStacktrace = false
            jdkHome = "%env.JDK_11_0%"
        }
        maven {
