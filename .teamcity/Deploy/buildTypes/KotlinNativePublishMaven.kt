...
object KotlinNativePublishMaven : BuildType({
    ...
    params {
        param("gradleParameters", """
            %globalGradleParameters% -Pbuild.number=%kotlin.native.version%
            -PkonanMetaVersion=%konanMetaVersion%
            -PkonanVersion=%konanVersion% -Pkotlin.native.enabled=true
        """.trimIndent())
    ...
    steps {
        ...
        gradle {
            ...
            buildFile = "build.gradle.kts"
            workingDir = "%teamcity.build.checkoutDir%/kotlin"
            gradleParams = "%gradleParameters% --no-parallel -Dorg.gradle.internal.publish.checksums.insecure=true -PsigningRequired=true -Psigning.gnupg.executable=gpg -Psigning.gnupg.keyName=%sign.key.id.new% -Psigning.gnupg.homeDir=%teamcity.build.checkoutDir%/kotlin/libraries/.gnupg -Psigning.gnupg.passphrase=%sign.key.passphrase.new% -Pdeploy-repo=%deploy-repo% -Pdeploy-url=%deploy-url% -PdeployVersion=${BuildNumber.depParamRefs["deployVersion"]} --no-scan --no-build-cache"
            enableStacktrace = false
            jdkHome = "%env.JDK_11_0%"
        }
    }
