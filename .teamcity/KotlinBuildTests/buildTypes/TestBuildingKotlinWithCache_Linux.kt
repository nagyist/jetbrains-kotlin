...
object TestBuildingKotlinWithCache_Linux : BuildType({
    ...
    params {
        param("gradleParameters", """%globalGradleParameters% -Pkotlin.build.cache.local.enabled=true -Pkotlin.build.cache.local.directory=%teamcity.build.checkoutDir%/build-cache -Pkotlin.build.cache.check.enabled=true "-Dscan.value.Configuration=Test building Kotlin with cache (Linux)"""")
