...
object KotlinNativePerformance_Tests_New_MM_Pre_commit_macos_x64 : BuildType({
    name = "🍎 Performance Tests MM.exp with Generation of Platform Libs (Native, Macos x86_64)"
    ...
    steps {
        ...
        gradle {
            name = "platform libs"
            tasks = ":kotlin-native:platformLibs:macos_x64-posix :kotlin-native:platformLibs:macos_x64-darwin :kotlin-native:platformLibs:macos_x64-Foundation :kotlin-native:platformLibs:macos_x64-posixCache :kotlin-native:platformLibs:macos_x64-darwinCache :kotlin-native:platformLibs:macos_x64-FoundationCache"
            buildFile = "build.gradle.kts"
            workingDir = "%teamcity.build.checkoutDir%/kotlin"
            gradleParams = """
                %gradleParameters% --parallel -Pbuild.number=%kotlin.native.version%
                -PkonanMetaVersion=%konanMetaVersion%
                -PkonanVersion=%konanVersion% -Pkotlin.native.home=%kotlin.native.test_dist% -Pkotlin.native.enabled=true -i -Pkotlin.native.performance.server.url=%kotlin.native.performance.server.url%
            """.trimIndent()
        ...
        gradle {
            ...
            gradleParams = """
                ...
                -PkonanMetaVersion=%konanMetaVersion%
                -PkonanVersion=%konanVersion% -Pkotlin.native.home=%kotlin.native.test_dist% -Pkotlin.native.enabled=true -i -Pkotlin.native.performance.server.url=%kotlin.native.performance.server.url% -Pkotlin_root=../..
            """.trimIndent()
            gradleWrapperPath = "../.."
            enableStacktrace = false
        }
        ...
        gradle {
            ...
            gradleParams = """
                %gradleParameters% --parallel -Pbuild.number=%kotlin.native.version%
                -PkonanMetaVersion=%konanMetaVersion%
                -PkonanVersion=%konanVersion% -Pkotlin.native.home=%kotlin.native.test_dist% -Pkotlin.native.enabled=true -i -Pkotlin.native.performance.server.url=%kotlin.native.performance.server.url% -PbuildNumberSuffix=NewMM
            """.trimIndent()
            gradleWrapperPath = "../.."
            enableStacktrace = false
        }
    ...
    dependencies {
        ...
        dependency(KotlinNative.buildTypes.KotlinNativeDist_macos_x64_DIST) {
            snapshot {
                onDependencyFailure = FailureAction.FAIL_TO_START
            }

            artifacts {
                artifactRules = "kotlin-native-macos-x86_64-%kotlin.native.version%.tar.gz!kotlin-native-macos-x86_64-%kotlin.native.version%/** => %kotlin.native.test_dist%"
            }
        }
    }
