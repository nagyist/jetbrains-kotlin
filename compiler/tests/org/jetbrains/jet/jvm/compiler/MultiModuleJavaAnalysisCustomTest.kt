...
public class MultiModuleJavaAnalysisCustomTest : UsefulTestCase() {
    ...
    private class TestModule(val name: String, val kotlinFiles: List<JetFile>, val javaFilesScope: GlobalSearchScope)
    ...
    fun testJavaEntitiesBelongToCorrectModule() {
        ...
        val analysis = JvmAnalyzerFacade()
                .setupAnalysis<TestModule>(GlobalContext(), environment.getProject(), modules,
                        // module c depends on [c, b, a], b on [b, a], a on [a], order is irrelevant
                                           { module -> modules.filter { it.name.first() <= module.name.first() } },
    ...
    private fun performChecks(analysis: AnalysisSetup<TestModule, JvmAnalyzer>, modules: List<TestModule>) {
        modules.forEach {
            ...
            val kotlinClass
                    = kotlinPackage.getMemberScope().getClassifier(Name.identifier("Kotlin${module.name.toUpperCase()}")) as ClassDescriptor
            checkClass(kotlinClass)

            val javaPackage = moduleDescriptor.getPackage(FqName.topLevel(Name.identifier("custom")))!!
            val classFromJava
                    = javaPackage.getMemberScope().getClassifier(Name.identifier(module.name.toUpperCase() + "Class")) as ClassDescriptor
