    ...
    private fun calcItemsToSkip(expression: JetExpression): Set<DeclarationDescriptor> {
        ...
        when(parent) {
            ...
            is JetWhenConditionWithExpression -> {
                ...
                if (subject != null) {
                    val subjectType = bindingContext[BindingContext.EXPRESSION_TYPE, subject]
                    if (subjectType != null) {
                        ...
                        if (classDescriptor != null && DescriptorUtils.isEnumClass(classDescriptor)) {
                            ...
                            for (anEntry in whenExpression.getEntries()) {
                                for (condition in anEntry.getConditions()) {
                                    if (condition is JetWhenConditionWithExpression) {
